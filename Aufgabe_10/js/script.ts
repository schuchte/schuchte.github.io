...
var checkedDOMElement: HTMLElement;
var uncheckedDOMElement: HTMLElement;
...
window.addEventListener("load", function(): void {
    ...
    checkedDOMElement = document.querySelector("#checked");
    uncheckedDOMElement = document.querySelector("#unchecked");
...
function drawListToDOM(): void {
    ...
    for (let index: number = 0; index < penguin.text.length; index++) {
         ...
         */
        todo.innerHTML =  "<span class='check " + penguin.checked[index] + "'><i class='fas fa-check'></i></span>"
                            + penguin.text[index] +
...
function updateCounter(): void {
    ...
    counterDOMElement.innerHTML = penguin.text.length + " in total";

    for (var i: number = 0; i < penguin.checked.length; i++ ) {
        if (penguin.checked[i] == true) {
            checked++;
        }
        else {penguin.checked[i] != true;  
            unchecked++;
        }}
    counterDOMElement.innerHTML = penguin.text.length + " in total | " + unchecked + "  open | " + checked + "  done";
}
...
function addTodo(): void {
    ...
    if (inputDOMElement.value != "") {
         ...
         */
        penguin.text.push(inputDOMElement.value);
        penguin.checked.push(false);
...
function toggleCheckState(index: number): void {
     ...
     */
    penguin.checked[index] = !penguin.checked[index];
...
function deleteTodo(index: number): void {
     ...
     */
    penguin.text.splice(index, 1);
    penguin.checked.splice(index, 1);
...
window.addEventListener("load", function(): void {
    ...
    artyom.addCommands({ 
        ...
        action: function(i: any, wildcard: string): void {
            ...
            penguin.text.push(wildcard);//wildcard wird gepusht//
            penguin.checked.push(false);//soll nicht gecheckt sein//
            drawListToDOM();
        }
    });
